...
template sudoku() {
    ...
    component rangeCmp_1[9][9];
    component rangeCmp_2[9][9];
        
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangeCmp_1[i][j] = RangeProof(5);

            rangeCmp_1[i][j].in <== puzzle[i][j];
            rangeCmp_1[i][j].range[0] <== 0;
            rangeCmp_1[i][j].range[1] <== 9;

            rangeCmp_2[i][j] = RangeProof(5);

            rangeCmp_2[i][j].in <== solution[i][j];
            rangeCmp_2[i][j].range[0] <== 0;
            rangeCmp_2[i][j].range[1] <== 9;

            rangeCmp_1[i][j].out === 1;
            rangeCmp_2[i][j].out === 1;
        }
    }
